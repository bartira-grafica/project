...
void loop() {
  ...
  if (lastSensorState == HIGH && sensorState == LOW) {
    ...
    char bufferContagem[256];
    serializeJson(jsonDocContagem, bufferContagem);

    client.publish("esteira/contagem", bufferContagem);
  }
  ...
  if (redOn) {
    ...
    // Criar JSON para MQTT (Esteira Parada)
    ...
    client.publish("esteira/deteccao", buffer);
    Serial.println("Publicação MQTT (Esteira Parada): " + String(buffer));
  } else {
    digitalWrite(LED_YELLOW_OUT, yellowOn ? HIGH : LOW);
    digitalWrite(LED_GREEN_OUT, yellowOn ? LOW : HIGH);
    digitalWrite(LED_RED_OUT, LOW);
  
    // Criar JSON para MQTT (Esteira em movimento)
    StaticJsonDocument<200> jsonDoc;
    jsonDoc["timestamp"] = millis();
